...
console.log('\n=== Top 20 buckets ===')
console.log(
  `${'bucket'.padEnd(40)} ${'raw bytes'.padStart(12)} ${'~gzipped'.padStart(12)} ${'% total'.padStart(8)}`,
)
...
for (const [name, b] of sortedBuckets.slice(0, 20)) {
  ...
  console.log(
    `${name.padEnd(40)} ${fmt(b.bytes).padStart(12)} ${fmt(gz).padStart(12)} ${pct.padStart(7)}%`,
  )
}
...
mdLines.push('')
mdLines.push(
  `> Generated by \`examples/docs/probes/source-attribution.ts\` on ${new Date().toISOString().slice(0, 10)}.`,
)
