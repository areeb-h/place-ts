...
// Dev-only graph scoping: tag every reactive node this island creates
// with the island's name, so the devtools can group the reactivity
...
function hydrateOne(el: HTMLElement): void {
  ...
  // Tag every reactive node created while this island instantiates +
  // mounts with the island's name (dev-only graph grouping). \`finally\`
  // pops even if instantiate/mount throws, so a failed island can't
  // leave the scope stack unbalanced.
  _pushDevScope(NAME)
  try {
...
export async function buildIslandBundles(
  ...
  if (!build.success) {
    throw new Error(`island-bundler: build failed:\n${build.logs.join('\n')}`)
  }
